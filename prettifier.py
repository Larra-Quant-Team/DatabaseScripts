class HtmlConverter():
    ...
    def calculate_table(self):
        ...
        for company in self.fields_info:
            self.fields_html += f"""<tr>
                                        <td> {company['id']} </td>
                                        <td> {companies.loc[company['id']]["Ticker Bloomberg"]} </td>
                                        <td> {companies.loc[company['id']].Industry_Sector} </td>
    ...
    def get_print(self):    
        ...
        mail_msg = f"Se ha consultado por {self.a_fields} campos"
        mail_msg += f"Los tiempos promedios de ejecución corresponden a:\n"
        mail_msg += f"- Tiempo prom. de consulta último registro: {self.avg_times_last}\n"
    ...
    def get_html(self):
        ...
        <body>
            <p>Hola,<br>
                Este email se ha generado automáticamente como reporte de la actualización de base de datos mongo
                ...
                <ul>    
                    <li> Tiempo prom. de consulta útimo registro: {self.avg_times_last} </li>
                ...
                Al final de este email se encuentra una tabla que indica cuantos campos fueron actualizados correctamente 
                para cada Equity. <br>
            </p>
